...
package tl.vspm {
    ...
    public class SWFAddress {
        ...
        public static var _availability:Boolean = ExternalInterface.available;
        ...
        public static function setValue(value:String):void {
            if (value == 'undefined' || value == null) value = '';
			if (value.lastIndexOf("/") == value.length - 1) value = value.substr(0, value.length - 1); //TŁ
		...
		public static function getCurrentSwfAddress(indPath: int = -1): String {
			...
			if (indPath == -1) {
				for (var i:uint = 0; i < pathNames.length; i++) {
					currentSwfAddress += (["", "/"][uint(i > 0)] + pathNames[i]);
				}
		...
		public static function getParametersString(objParams: Object = null): String {
			var strParameters: String = "";
			if (objParams == null) strParameters = "?" + SWFAddress.getQueryString();
			else {
				var i: uint = 0;
				for (var nameParam: String in objParams) strParameters += (["?", "&"][uint(i++ > 0)] + nameParam + "=" + objParams[nameParam]);
			}
