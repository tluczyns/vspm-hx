package tl.loader {
    ...
    public class URLLoaderExt extends URLLoader {
		...
		public static function parseXMLToObject(xmlNode: XML): Object {
			...
			var xmlListAttributes: XMLList = xmlNode.attributes(); //xmlNode.@ * ;
			...
			var xmlListElements: XMLList = xmlNode.elements(); //xmlNode.*
		...
		static public function sortXML(source:XML, elementName:Object, fieldName:Object, options:Object = null):XML {
			...
			var xmlList:XMLList = source.elements(elementName); //elements().(name() == elementName);
			...
			var xmlListExclude: XMLList = source.elements();
			for (var i: int = 0; i < xmlListExclude.length(); i++) {
				if (xmlListExclude[i].name() == elementName) {
					delete xmlListExclude[i];
					i--;
				}
			}
			xmlList = sortXMLList(xmlList, fieldName, options);
			xmlList += xmlListExclude;
			source.setChildren(xmlList);
			return source;
		}
